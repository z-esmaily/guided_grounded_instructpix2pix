...
# Spatial words for directional logic
spatial_words = {
    'left': ['left'],
    'right': ['right'],
...
# Pronouns and their replacements
...
class ExternalMaskExtractor:
    def __init__(self, device='cuda:0',
                 target_predictor_model_path=None,
                 size_predictor_model_path=None,
                 sam_path=None):
        self.device = device

        # Load models using the provided paths
        if target_predictor_model_path:
            self.target_predictor = TargetPredictor(model_path=target_predictor_model_path)
        else:
            raise ValueError("target_predictor_model_path must be provided.")

        if size_predictor_model_path:
            # SizePredictor handles a list of paths if ensemble
            self.size_predictor = SizePredictor(model_paths=size_predictor_model_path, device=self.device)
        else:
            raise ValueError("size_predictor_model_path must be provided.")

        if sam_path:
            self.sam_predictor = SamPredictor(build_sam(checkpoint=sam_path).to(device))
        else:
            raise ValueError("sam_path must be provided.")
    ...
    def replace_pronoun(self, target, target_phrase, image, verbose=False):
        ...
        if target_lower in male_pronouns:
            ...
            if any(dino_scores.values()):  # If at least one replacement had detections
                ...
                # Fallback to "them" if no good detections (default choice)
                default_noun = "them"
    ...
    def _grounded_sam_predict(self, image, first_target_phrase, first_target, second_target_phrase, second_target, final_sentence, direction):                    
        ...
        # Predict without "the" prefixed if the prefixed exist
...
##################################################################################################
        ...
        if second_target != "No second target found":
            # Call _dino_predict with both original "the" prefixed and "not the" target phrase for second target
            all_boxes2 = []
            all_scores2 = []
            all_phrases2 = []
            ...
            if boxes2_curr is not None:
                print("Shape of Boxes for", second_target_phrase, " : ", boxes2_curr.shape)
                all_boxes2.append(boxes2_curr)
                all_scores2.append(scores2_curr)
                all_phrases2.append(phrases2_curr)

            # Predict without "the" prefixed if the prefixed exist for second target
            if second_target_phrase.lower().startswith("the "):
                ...
                if boxes2_notthe is not None:
                    print("Shape of Boxes for", notthe_second_target_phrase, " (without the) : ", boxes2_notthe.shape)
